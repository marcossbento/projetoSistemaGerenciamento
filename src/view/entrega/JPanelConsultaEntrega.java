...
public class JPanelConsultaEntrega extends javax.swing.JPanel implements GetSelectedObjectService{
  
    private Entrega data;
    private final String query = "SELECT * from Entregas ";
    private ResultSetTableModel result;
    private final TableRowSorter<TableModel> filter;
    private Funcionario logado;
    public JPanelConsultaEntrega(Entrega data,Funcionario logado, Boolean select) throws SQLException {
        initComponents();
        this.logado = logado;
        ...
        jTableEntrega.setModel(result);
        
        filter = new TableRowSorter<TableModel>(result);
        jTableEntrega.setRowSorter(filter);
    }
    ...
    private void addEntrega(){
        JFrameEntregaCRUD crud = new JFrameEntregaCRUD(null,logado);
    ...
    private void changeEntrega(){
       int row = jTableEntrega.getSelectedRow();
        
        if(row == -1) return;
        data = new Entrega();
        data.setIdEntrega((int) result.getValueAt(row, 0));
        
        try {
            data.load();
            
            JFrameEntregaCRUD crud = new JFrameEntregaCRUD(data,logado);
    ...
    private void initComponents() {
        ...
        jTableEntrega.setFont(new java.awt.Font("Segoe UI", 1, 14)); // NOI18N
        ...
        jTableEntrega.setOpaque(false);
        jTableEntrega.setRowHeight(30);
    ...
    public DataAccessObject getSelected() {
        ...
        entrega.setIdEntrega((int) result.getValueAt(row, 0));
